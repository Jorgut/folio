#!/usr/bin/env node
/**
 * Export Folio deck to InDesign-friendly PDF.
 *
 * Differences from print PDF:
 *   - No PNG image overlay (images stay native/editable)
 *   - Retains bleed + crop marks for InDesign import
 *   - Text is selectable
 *
 * Usage:
...
function toInDesignOutputPath(htmlPath) {
  const name = basename(htmlPath, '.html');
  return join(resolve(htmlPath, '..'), `${name}.indesign.pdf`);
}
...
async function preparePrintableDeck(page) {
  ...
  await page.evaluate(({ bleedPx }) => {
    ...
    for (const s of slides) {
      ...
      s.classList.add('print-slide', 'active');
...
/**
 * Render PDF via Playwright, then add crop marks + trim boxes using pdf-lib.
 * Unlike the print PDF, this does NOT re-embed PNG overlays — images stay
 * native so InDesign can handle them.
 */
async function renderInDesignPdf(fileUrl, outputPath) {
  const browser = await chromium.launch({ headless: true, channel: 'chrome' });
  try {
    ...
    const tempPdfPath = join(join(tmpdir(), 'folio-indesign-temp'), 'raw.pdf');
    mkdirSync(join(tmpdir(), 'folio-indesign-temp'), { recursive: true });
    ...
    // Add crop marks + trim boxes via pdf-lib
...
async function main() {
  ...
  if (!htmlPath || !existsSync(htmlPath)) {
    console.error('Usage: node scripts/export-indesign.mjs <path/to/index.html>');
    process.exit(1);
  }
  const absPath = resolve(htmlPath);
  const outputPath = toInDesignOutputPath(absPath);
  const imagesDir = mkdtempSync(join(tmpdir(), 'folio-indesign-images-'));
  ...
  const slideCount = await renderInDesignPdf(toFileUrl(tempHtml), outputPath);
  console.log(`  ${slideCount} slides`);
  console.log(`[3/3] Done → ${outputPath}`);
}
